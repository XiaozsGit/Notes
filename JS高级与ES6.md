...
- **注意这两个对象不可以混淆的调用，只能各调各的** （原型对象属于静态成员，但是，当实例对象需要的时候却可以调用到原型对象中的方法，这里是 原型在做桥梁）
...
- 原型对象的作用：可以节省内存空间，当一个方法或变量在所有的同类型对象中都要 使用 或 者拥有 时，可以将其放在其原型对象中，这样只需要创建一个函数就好，当实例对象需要使用方法，而其自身又没有时，就会去他的原型对象中查找，如果有就可以执行了，如果没有就继续往上找，直到找到了或者到达顶部（Object）却依旧没有而返回 is not defined

- 注意这个过程中原型所带来的作用，上面的这两个东西东隐含了一个东西（实例成员调用到了静态成员的属性方法，原型对象）按理是不能直接调用的，这里的原型起到了一个桥梁作用
