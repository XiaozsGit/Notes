...
- 注意在某些逻辑中改变的数据是否是 data 中定义并绑定了的数据，有些方法会返回一个数据，而我们会对返回的数据进行操作（不对原数据进行操作），所以不会自动触发 视图更新
...
- 在这些函数内部添加 事件处理逻辑，可以对接口数据初始化，等（在vue实例中定义了初始化函数后，没有直接的环境来调用该方法（vue实例只是一个对象），vue实例中都是键值对，不能直接加括号删除，在外部使用 vm.方法 又有点不伦不类，所以可以在生命周期的方法中调用）（这是框架为我们提供好的一个东西）
